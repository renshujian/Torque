...
namespace Modbus
{
    public class Message
    {
        ...
        public static Message FromBytes(ArraySegment<byte> bytes)
        {
            if (bytes.Count < 8)
            {
                throw new ModbusException("modbus消息长度要不少于8字节");
            }
            ...
            {
                throw new ModbusException($"modbus消息协议标识要为{PROTOCOL_ID}");
            }
            if (length != bytes.Count - 6)
            {
                throw new ModbusException("modbus消息长度标识与收到的字节数不符");
            }
            var list = (IReadOnlyList<byte>)bytes;
            return new Message
            {
